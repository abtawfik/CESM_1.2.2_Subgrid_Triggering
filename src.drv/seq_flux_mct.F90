...
!--------------------------------------------------------------------------
  ...
  real(r8), allocatable ::  tbm  (:)     ! atm convective threshold  abt added
  ...
  real(r8), allocatable ::  convfrac (:)     ! convective fraction       abt added
  ...
  integer :: index_a2x_Sa_tbm      !<===== abt added
  ...
  integer :: index_xao_So_convfrac !<=====  abt added
...
!===============================================================================
  ...
  subroutine seq_flux_init_mct( cdata, fractions)
    ...
    allocate(tbm(nloc),stat=ier)                        !abt added
    if(ier/=0) call mct_die(subName,'allocate tbm',ier) !abt added
    ...
    allocate(convfrac(nloc),stat=ier)                         !abt added
    if(ier/=0) call mct_die(subName,'allocate convfrac',ier)  !abt added
...
!===============================================================================
  ...
  subroutine seq_flux_initexch_mct(cdata_a,cdata_o)
    ...
    allocate(convfrac(nloc_a2o),stat=ier)                     !abt added
    if(ier/=0) call mct_die(subName,'allocate convfrac',ier)  !abt added
...
!===============================================================================
  ...
  subroutine seq_flux_atmocnexch_mct( cdata_a, cdata_o, a2x, o2x, xao_a, xao_o, fractions_a, fractions_o)
    ...
    integer(in) :: index_convfrac      !<====== abt added
    ...
    if (dead_comps) then
       do n = 1,nloc_a2o
          ...
          tbm (n) = 300.0_R8 ! atm conv thresh temperature  ~ Kelvin   !abt added but not sure what this is?????
       enddo
    else        
       ...
       do n = 1,nloc_a2o
          ...
          tbm (n) = a2x_e%rAttr(index_a2x_Sa_tbm ,ia)    !abt added
    ...
    call shr_flux_atmocn (nloc_a2o , zbot , ubot, vbot, thbot, &
                          shum , dens , tbot, tbm , uocn, vocn , &      !<======  abt added
                          tocn , emask, sen , lat , lwup , &
                          evap , taux , tauy, tref, convfrac, qref , &  !<======  abt added
    ...
    index_convfrac = mct_aVect_indexRA(xaop_ae,"So_convfrac")   !<===== abt added
    ...
    do n = 1,nloc_a2o
       ...
       xaop_oe%rAttr(index_convfrac ,io) = xaop_oe%rAttr(index_convfrac ,io) + convfrac(n)* wt !<====  abt added
...
!tcx   wt = sMato2a%data%rAttr(kw,n)
       ...
       xaop_ae%rAttr(index_convfrac ,ia) = xaop_ae%rAttr(index_convfrac  ,ia) + convfrac(n)* wt !<===== abt added
...
!===============================================================================
  ...
  subroutine seq_flux_atmocn_mct( cdata, a2x, o2x, xao)
    ...
    if (first_call) then
       ...
       index_xao_So_convfrac = mct_aVect_indexRA(xao,'So_convfrac') !<==== abt added
       ...
       index_a2x_Sa_tbm    = mct_aVect_indexRA(a2x,'Sa_tbm' )   !<==== abt added
    ...
    if (dead_comps) then
       do n = 1,nloc
          ...
          dens(n) =   1.0_R8 ! atm density                ~ kg/m^3
          tbot(n) = 300.0_R8 ! atm temperature            ~ Kelvin
          tbm (n) = 300.0_R8 ! atm conv threshold         ~ Kelvin   abt added not sure what this is ?????
       enddo
    else	
       do n = 1,nloc
          if (mask(n) /= 0) then	
             ...
             tbm (n) = a2x%rAttr(index_a2x_Sa_tbm ,n)   !<===== abt added
...
!                         duu10n,ustar, re  , ssq, missval = 0.0_r8 )
    ...
    do n = 1,nloc
       if (mask(n) /= 0) then	
          ...
          xao%rAttr(index_xao_So_convfrac,n)     = convfrac(n) !<=== abt added

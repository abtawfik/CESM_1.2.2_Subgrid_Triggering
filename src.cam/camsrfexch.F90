...
!---------------------------------------------------------------------------
  ...
  type cam_out_t 
     ...
     real(r8) :: tbm(pcols)          ! buoyant mixing potential temperature convective threshold (K) abt added
...
!---------------------------------------------------------------------------
  ...
  type cam_in_t    
     ...
     real(r8) :: convfrac(pcols)         ! fraction of gridcell that is convective (decimal 0-1) !<===== abt added
...
!
    ...
    do c = begchunk,endchunk
       ...
       cam_in(c)%convfrac (:) = 0._r8    !abt added

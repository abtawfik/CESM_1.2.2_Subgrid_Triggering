...
!
    ...
    call addfld ('CONVFRAC',  'unitless',       1, 'A', 'Fraction of sub-grid triggering convection', phys_decomp)
    call addfld ('TBM'     ,  'K'       ,       1, 'A', 'Convective threshold passed to CLM to return CONVFRAC', phys_decomp)
...
subroutine zm_conv_tend(pblh    ,mcon    ,cme     , &
     ...
     ztodt   , &
     jctop   ,jcbot , &
     state   ,ptend_all   ,landfrac,  pbuf, convfrac)  !abt added)
...
!
   ...
   call zm_convr(   lchnk   ,ncol    , &
                    ...
                    lengath(lchnk) ,ql      ,rliq  ,landfrac, convfrac   )   !abt added   )

   call outfld('CAPE', cape, pcols, lchnk)        ! RBN - CAPE output
   call outfld('CONVFRAC', convfrac, pcols, lchnk)        ! abt - fraction of grid with convection

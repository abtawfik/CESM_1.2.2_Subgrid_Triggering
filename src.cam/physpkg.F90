...
subroutine tphysbc (ztodt,               &
    ...
    !*** abt added below ***
    !***********************
    use hcf_trigger,     only: hcf_conv_trigger
    !***********************
    !*** abt added above ***
    !***********************
    ...
    real(r8) :: convfrac_in(pcols)             ! private convective fraction <-- cam_in%convfrac abt added
    ...
    ! Convective threshold (tbm) is passed to the surface and compared against T2M
    ...
    !            convection
    !-------------------------------------------------------------------------------
    !----- Because there is no restart options yet; call this block only for the first timestep
    !----- to ensure the there is a convective fraction calculate on the grid average
    !----- subsequent timesteps will use the convective fraction calculate at subgrid tiles from CLM
    !----- and average gridcell T2M when there is more than 30% ocean/ice
    !
    !
    if( nstep == 0  ) then
       call hcf_conv_trigger( ncol, state%t, state%pmid*0.01_r8, state%q(:,:,1), cam_out%tbm )
       where( (cam_out%tbm(:ncol)-cam_in%tref(:ncol)).gt.0 )
          convfrac_in(:ncol) = 0._r8
       elsewhere
          convfrac_in(:ncol) = 1._r8
       endwhere
    else
       convfrac_in(:ncol)  =  cam_in%convfrac(:ncol)
    end if
    !
    !
    !----- If first call (initial) then calc convective fraction on the gridcell
    !----- by comparing TBM to total gridcell average T2M
    !** abt added above
    !----- Make sure there is no convection over ice b/c it is not implemented yet and
    !----- unlikely anyway;  something to note for the future in fully coupled runs
    where( convfrac_in.lt.0   .or.  convfrac_in.gt.100 )
         convfrac_in  =  0._r8
    endwhere
    !===============================================================================
    ...
    call convect_deep_tend(  &
         ...
         rliq,    &
         ztodt,   &
         state,   ptend, cam_in%landfrac, pbuf, convfrac_in )   ! abt added 
    ...
    !end do
    call hcf_conv_trigger( ncol, state%t, state%pmid*0.01_r8, state%q(:,:,1), cam_out%tbm )
    call outfld('TBM', cam_out%tbm, pcols, lchnk)        ! abt - fraction of grid with convection
    !===============================================================================
    !** abt added above
    !===============================================================================

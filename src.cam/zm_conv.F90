...
subroutine zm_convr(lchnk   ,ncol    , &
                    ...
                    lengath ,ql      ,rliq    ,landfrac, convfrac )     !abt added)
!----------------------------------------------------------------------- 
! 
! Purpose: 
...
!
   ...
   real(r8), intent(in) :: convfrac(pcols)      ! fraction of grid with convection   <==== abt added
!
! output arguments
!
...
!
   lengath = 0
   do i=1,ncol
      if (convfrac(i) > 0._r8 .and. cape(i) > capelmt) then    !abt changed
         lengath = lengath + 1
         index(lengath) = i
      end if
   end do
...
!
   ...
   !abt added below
   !----------------------------------------------------------------
   !  Modify the total mass flux by the fraction of the surface
   !  that is convectiving.  This is done by multiplying by
   !  the convective fraction calculated from the HCF framework
   !  Specifically by comparing convective threshold (TBM) to
   !  the surface potential temperature for each PFT or surface tile
   !----------------------------------------------------------------
   do i=1,lengath
      mb(i) = mb(i) * convfrac(i)
   end do
   !abt added above

...
SUBROUTINE shr_flux_atmOcn(nMax  ,zbot  ,ubot  ,vbot  ,thbot   ,          & 
           &               qbot  ,rbot  ,tbot  ,tbm   ,us      , vs   ,   &  !<====== abt added
...
! !INPUT/OUTPUT PARAMETERS:
   ...
   real(R8)   ,intent(in) :: tbm  (nMax) ! atm Convect Threshold (K)      !<====== abt added
   ...
   real(R8),intent(out)  ::  convfrac (nMax) ! fraction convecting    (fraction)    <==== abt added
...
!-------------------------------------------------------------------------------
   ...
   DO n=1,nMax
     if (mask(n) /= 0) then
        ...
        !***************************
        !----------------------------------------------------------------
	! computes fraction of ocean that has triggered moist convection
        !----------------------------------------------------------------
        if( (tbm(n)-tref(n)).le.0 ) then
            convfrac(n) =  1.0_R8
	    else
            convfrac(n) =  0.0_R8
	    end if
	...
	!***   abt added above   ***
        ...
        convfrac(n) = spval  ! convective fraction (fraction)   <==== abt added

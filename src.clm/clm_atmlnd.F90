...
!----------------------------------------------------
  ...
  type, public :: atm2lnd_type
     real(r8), pointer :: tbm   (:)       => null() !convective threshold (Kelvin)   !!!abt added
...
!----------------------------------------------------
  ...
  type, public :: lnd2atm_type
     ...
     real(r8), pointer :: convfrac(:)     => null() !convective fraction     !!! abt added
...
!------------------------------------------------------------------------
  ...
  allocate(a2l%tbm   (beg:end))  !abt added
  ...
  a2l%tbm   (beg:end) = ival   !abt added
...
!------------------------------------------------------------------------
  ...
  allocate(l2a%convfrac (beg:end))    !<========= abt added
  ...
  l2a%convfrac(beg:end)               = ival    !abt added
...
!EOP
  integer :: g                           ! indices
  integer :: p, c                        ! indices  abt added
  real(r8), allocatable ::  contrig(:)   ! temporary pft level trigger (0=no convection; 1=convection)  abt added
  real(r8)              ::  pot2m        ! temporary pft level 2m potential temperature -- abt added
...
!          p2c_scale_type='unity', c2l_scale_type= 'urbanf', l2g_scale_type='unity')
  ...
  !***** abt added below *****
  !***************************------------------------------------------------------
  !  Add to calculate the fraction of the gridcell that is triggering convection
  !  Each pft determines whether convection is triggered by comparing 2m potential
  !  temperature to the convective threshold (tbm from the atmosphere).
  !  If TBM <= POT2M then convection is triggered and contrig=1; otherwise contrig=0
  !  This information is passed back to CAM to inform the deep convective scheme
  !---------------------------------------------------------------------------------
  if(.not.allocated(contrig)) allocate(contrig(begp:endp))
  do p = begp,endp
     g = pft%gridcell(p)
     c = pft%column  (p)

     pot2m  =  pes%t_ref2m(p) * ((100000./cps%forc_pbot(c))**(rair/cpair))
     if( clm_a2l%tbm(g) .le. pot2m ) then
         contrig(p) = 1._r8
     else
         contrig(p) = 0._r8
     end if
  ...
  call p2g(begp, endp, begc, endc, begl, endl, begg, endg, &
           contrig, clm_l2a%convfrac,                  &
           p2c_scale_type='unity', c2l_scale_type= 'unity', l2g_scale_type='unity')

  if(allocated(contrig)) deallocate(contrig)
  !***************************
  !***** abt added above *****
  !***************************
